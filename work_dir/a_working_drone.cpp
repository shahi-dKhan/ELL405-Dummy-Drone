// drone_core.cpp
// PHASE: ADVANCED PROFILER (Fixed Compilation Errors)
...
struct SystemStats {
    ...
    long vision_preempts = 0; // The struct member name
...
void task_monitor(int core_mode) {
    ...
    cout << "| Time(us) | Preempts   | FPS | Preempts        | Active | Preempts    |" << endl;
    cout << "------------------------------------------------------------------------" << endl;

    while (system_running) {
        std::this_thread::sleep_for(std::chrono::seconds(1));

        long f_time, f_pre, v_fps, v_pre, e_act, e_pre;
        ...
        cout << "| " << setw(8) << f_time << " | " << setw(10) << f_pre 
             << " | " << setw(3) << v_fps << " | " << setw(13) << v_pre 
